...
contract StakingDapp is Ownable, ReentrancyGuard{
    //because we have to do the transfer of the token
    using SafeERC20 for IERC20;

    //going to keep certain information about user that how many tokens they have taken
    //lockUntil means when he deposited the token
    struct UserInfo{
        uint amount;
        uint lastRewardAt;
        uint lockUntil;
    ...
    uint decimals = 10 ** 18;
    // we have to count the total number of pools which admin will create 
    uint public poolCount;

    PoolInfo[] public poolInfo;

    mapping (address=> uint) public depositedTokens; //for token deposit
    ...
    function addPool(IERC20 _depositToken, IERC20 _rewardToken, uint _apy, uint _lockDays) public onlyOwner{
        poolInfo.push(PoolInfo({
            depositToken: _depositToken, //0x3328358128832A260C76A4141e19E2A943CD4B6D
            rewardToken: _rewardToken, //0x5e17b14ADd6c386305A32928F985b29bbA34Eff5
            depositedAmount: 0,
            apy:_apy,
            lockDays:_lockDays
    ...
    // why used array for the poolInfo? could've used mapping
    function deposit(uint _pid, uint _amount) public nonReentrant{
        ...
        //give the pending reward if remaning any
        if(user.amount>0){
            uint pending = _calcPendingReward(user,pool);
    ...
    function withDraw(uint _pid, uint _amount) public nonReentrant{
        ...
        require(user.lockUntil<=block.timestamp,"Lock is Active you cannot withdraw at this moment!");

        PoolInfo storage pool = poolInfo[_pid]; //pool struct
        uint pending = _calcPendingReward(user,pool);
    ...
    //This particular function will give us what reward user have in the contract

    function _calcPendingReward(UserInfo storage user,PoolInfo storage pool)internal view returns(uint){
        ...
        return user.amount * daysPassed / 365 / 100 * pool.apy; //calculating the reward
    }

    function pendingReward(uint _pid, address _user) public view returns(uint){
        UserInfo storage user = userInfo[_pid][_user];
        PoolInfo storage pool = poolInfo[_pid]; 
        return _calcPendingReward(user,pool);

    }

    // This function will be called by owner of the contract to withdraw the token out of the staking contract

    function sweep(address token, uint amount)external onlyOwner{
        uint token_balance = IERC20(token).balanceOf(address(this)); //with this we will be able to find out how tokens are at stake in the contract for this particular token
